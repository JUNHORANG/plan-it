...
- **공통 자원은 루트 절대경로(`/shared/...`)로 참조**한다. 폴더가 3단계까지 깊어져 상대경로(`../../../`)가 지저분해지므로, **프로젝트 루트를 정적 서버로 서빙**하는 것을 전제로 한다.
- **정적 서버 필수.** ES module `import`와 `fetch`는 `file://`에서 동작하지 않는다.
  - VS Code **Live Server**, 또는 `python3 -m http.server`, 또는 `npx serve` 중 택1.
...
<html lang="ko">
  <head>
    ...
    <link rel="stylesheet" href="/user/plans/index.css" />
    <!-- 페이지 전용 CSS -->
  </head>
  <body>
    <div id="header"></div>
    <!-- 공통 헤더 mount 지점 (앱바 페이지는 #app-bar) -->
    ...
    <!-- 바텀시트/모달/드로워 mount 지점 -->
    <script type="module" src="/user/plans/index.js"></script>
    <!-- 페이지 전용 JS -->
  </body>
</html>
```
...
- [ ] `shared/css/variables.css` — 전역 변수, 리셋
- [ ] `shared/css/layout.css` — 헤더/앱바/레이아웃 공통
