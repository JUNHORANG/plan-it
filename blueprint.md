...
29. **회원 탈퇴 인증 시간 초과 이동 도메인 오타 + Figma 미확인** — spec.md "회원 탈퇴" 6번 "인증 시간을 넘기면 도메인 /planit으로 이동한다"의 `/planit`은 실제 존재하는 라우트가 아니라(1번·3번과 같은 유형의 오타) `/user/plans/`(홈)로 해석해 구현. 전용 Figma(4589:1159)도 계속 API rate limit(429)로 실측 못 받아와 spec.md 문구 + signup 이메일 인증 스텝(§9 23번)과 동일한 컴포넌트/흐름(공용 `shared/components/input.js` 비활성화 상태로 로그인 이메일 읽기 전용 표시, 5분 타이머+연장)을 그대로 재사용해 우선 구현 — 나중에 Figma 확인되면 세부 스타일만 맞추면 됨. **2026-07-20 갱신(Figma 실측 완료, 4589:1159 + 확인 모달 포함 4646:1134)**: (1) 상단이 메인 헤더(햄버거+종)가 아니라 뒤로가기+제목 앱바였음 — `shared/components/app-bar.js`로 교체, 본문 중복 h1 제목 삭제. (2) 인증 메일 발송 직후 이메일 입력창에 체크 아이콘이 뜬다(signup과 동일하게 "발송 성공" 시점에 표시, 실제 링크 클릭 확인 여부와 무관) — `shared/components/input.js`는 그대로 두고 이 화면에서만 로컬로 아이콘 엘리먼트를 추가(체크 아이콘 재사용 가능한 형태로 shared화하지 않은 이유는 signup-input.js 주석과 동일: 화면마다 위치 공식이 달라 공유하면 서로 깨짐). 비활성화 입력창도 회색으로 죽지 않고 기본 배경(`--color-input-bg`) 유지가 실측값. (3) CTA("계정 탈퇴") 클릭 시 바로 탈퇴하지 않고 확인 모달이 한 번 더 뜬 뒤 확정 — 공용 `modal.js` 재사용. Figma는 이 모달에서 취소 버튼(우측)을 accent 주황, 실제 탈퇴 버튼(좌측)을 회색으로 반대로 칠했는데, `modal.js`는 `cancelLabel`=회색/좌+닫기전용, `confirmLabel`=accent/우+`onConfirm` 실행 고정 구조라 그대로 재현하려면 공용 컴포넌트를 화면 하나 때문에 고쳐야 함 — 대신 signup 뒤로가기 모달과 동일한 기존 관례(실제로 상태를 바꾸는 동작을 confirm/accent 쪽에 매핑)를 따라 `cancelLabel:"취소하기"`(회색/좌)·`confirmLabel:"계정 탈퇴"`(accent/우, 실제 삭제 실행)로 구현 — Figma 색 배치와 다른 의도적 결정. 이메일 인증도 signup과 동일하게 SMTP 없이 6자리 코드 대신 확인 링크 방식(`sendResignVerification`이 `emailRedirectTo: /user/auth/resign/verified`로 발송, `is_resign_verified`류 전용 RPC로 확인 여부만 조회 — signup의 `is_email_verified`는 "언젠가 한 번이라도 확인됐는지"라 이미 가입된 계정엔 항상 true라 재사용 불가, `profiles.resign_confirmed_at`을 매 시도마다 초기화→재확인하는 별도 RPC 3종 사용). 탈퇴 자체는 `delete_own_account()` RPC(`auth.users` 삭제, `profiles.id`가 `auth.users.id`에 CASCADE라 plans/orders/notifications까지 함께 삭제)로 처리, 성공 시 로그인(`/`)으로 이동.
